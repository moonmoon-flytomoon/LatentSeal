...
from packages.ldm.modules.attention import MemoryEfficientCrossAttention
...
class combine_wm(nn.Module):
    def __init__(self, in_channels, out_channels, watermark_embedding_len=384,kernel_size=3,gn_group=32):
        ...
        self.layer = nn.Sequential(
            nn.Conv2d(in_channels=in_channels+in_channels, out_channels=out_channels, kernel_size=kernel_size, padding=1),
    ...
    def forward(self, x,wm):
        ...
        # wm = wm.repeat(1, 1, 2, 2)
        x = self.layer(torch.cat([x,wm],dim=1))
        x = self.zero_conv_1(x)
        x = self.dropout_watermark(x)
...
class WaterMarkDecoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
                 resolution, z_channels, give_pre_end=False, tanh_out=False, use_linear_attn=False,
                 attn_type="vanilla", cfg=None,**ignorekwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
        self.cfg = cfg
        self.ch = ch
        ...
        # middle
        self.mid = nn.Module()
        self.mid.block_1 = WaterMarkResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout)
        self.mid.attn_1 = make_attn(block_in, attn_type=attn_type)
        self.mid.block_2 = WaterMarkResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks+1):
                ...
                # if (i_level==0 or i_level==1) and i_block==1:
                    block.append(WaterMarkResnetBlock(in_channels=block_in,
                                                      out_channels=block_out,
                                                      temb_channels=self.temb_ch,
                                                      dropout=dropout,
                                                      watermark=True,
                                                      wm_channels=self.cfg.watermark_decoder.wm_channels,
                                                      gn_group=32))
                else:
                    block.append(WaterMarkResnetBlock(in_channels=block_in,
                                                      out_channels=block_out,
    ...
    def forward(self, z,watermark=None):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                if i_level == 0 and i_block == 1:
                    h = self.up[i_level].block[i_block](h, temb,watermark[0])
                # elif i_level == 1 and i_block == 1:
                #     h = self.up[i_level].block[i_block](h, temb,watermark[1])
                # if i_level == 2 and i_block == 0:
                #     h = self.up[i_level].block[i_block](h, temb,watermark[2])
                else:
                    h = self.up[i_level].block[i_block](h, temb)
                if len(self.up[i_level].attn) > 0:
                    h = self.up[i_level].attn[i_block](h)
